...
async function handleStartLinking(update: any) {
  ...
  if (!code) {
    await sendMessage(chatId, 'Please send your link code as: /start 123456')
    return true
  }
  ...
  const linkedRole = String(user.role || 'user')
  if (linkedRole === 'seller') {
    await sendMessage(chatId, '✅ Telegram connected successfully! You will now receive order notifications and controls.')
  } else {
    await sendMessage(chatId, '✅ Telegram connected successfully! You will now receive account and order notifications.')
  }
  return true
}
...
async function handleSellerCommand(update: any) {
  ...
  if (text === '/start') {
    await sendMessage(chatId, 'Welcome! To connect your seller account, generate code in dashboard then send: /start 123456')
    return true
  }
  ...
  if (!seller) {
    await sendMessage(chatId, 'Account not linked. Generate code in dashboard and send: /start 123456')
    return true
  }
