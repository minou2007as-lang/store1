  ...
  return (
    <div className="flex-1 px-3 py-4 sm:px-6 sm:py-6 lg:px-8 lg:py-8">
      <div className="mb-6 sm:mb-8">
        <div className="mb-6 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div>
            ...
            <p className="mt-2 text-sm text-slate-600 sm:text-base">Track and manage your game service orders.</p>
          </div>
        ...
        {['All', 'Pending', 'In Progress', 'Completed'].map((status) => (
          <button
            ...
            className={`px-4 py-2 font-medium text-sm border-b-2 border-transparent hover:border-blue-600 hover:text-blue-600 transition-colors ${
              statusFilter === status.toLowerCase().replace(' ', '_') ? 'border-blue-600 text-blue-600' : ''
            }`}
            onClick={() => setStatusFilter(status === 'All' ? 'all' : status.toLowerCase().replace(' ', '_'))}
          >
            {status}
          </button>
        ))}
      ...
      <div className="space-y-4">
        {loading ? (
          <Card>
            <CardContent className="p-12 text-center text-slate-500">Loading orders...</CardContent>
          </Card>
                ...
                <div className="grid grid-cols-1 md:grid-cols-12 gap-4 items-start">
                  <div className="md:col-span-4">
                    ...
                    <p className="text-sm text-slate-600 mt-1">
                      {order.assigned_seller_id ? 'Seller assigned' : 'Waiting for seller'}
                    </p>
                  </div>
          ...
          <Card>
            <CardContent className="p-12 text-center">
              ...
              <Link href="/dashboard/post-task">
                <Button className="w-full sm:w-auto">Post Your First Task</Button>
              </Link>
            </CardContent>
          </Card>
        )}
