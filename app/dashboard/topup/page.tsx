...
export default function TopupPage() {
      ...
      if (isAdmin) {
        ...
        if (seenAdminTopupIds.current.size === 0) {
          ...
          if (freshIds.length > 0 && silent) {
            toast.success(`${freshIds.length} new top-up request${freshIds.length > 1 ? 's' : ''} received`);
          }
  ...
  const copyInstructions = async () => {
    ...
    if (!selectedMethod?.instructions) {
      toast.error('No instructions available');
      return;
    }
    ...
    try {
      ...
      toast.error('Unable to copy instructions');
    } finally {
  ...
  const submitTopup = async (event: React.FormEvent) => {
    ...
    if (!isCustomer) {
      toast.error('Only customers can submit top-up requests');
      return;
    }
    ...
    try {
      ...
      toast.success(data?.message || 'Top-up request submitted successfully');
      setProofFile(null);
  ...
  const moderateTopup = async (id: string, action: 'approve' | 'reject') => {
    ...
    try {
      ...
      toast.success(data?.message || `Top-up ${action}d successfully`);
      if (action === 'reject') {
      ...
      console.error(error);
      toast.error(error instanceof Error ? error.message : `Failed to ${action} top-up`);
    } finally {
