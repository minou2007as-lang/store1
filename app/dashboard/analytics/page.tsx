...
export default function AnalyticsPage() {
    ...
    return (
      <div className="flex-1 p-8">
        <div className="mb-8">
          ...
          <p className="text-slate-600 mt-2">Admin access is required to view platform analytics.</p>
        </div>
        <Card className="text-center py-8">
          <p className="text-slate-600">You do not have permission to access analytics.</p>
        </Card>
      </div>
    );
  }
  ...
  return (
    <div className="flex-1 p-8">
      <div className="mb-8">
        <h1 className="text-3xl font-bold text-slate-900">Analytics Dashboard</h1>
        <p className="text-slate-600 mt-2">Platform performance and metrics loaded from the backend.</p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
        <Card>
          ...
          <CardContent>
            ...
            <p className="text-xs text-slate-600 mt-1">Last 6 months</p>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent>
            ...
            <p className="text-xs text-slate-600 mt-1">Total registered users</p>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent>
            ...
            <p className="text-xs text-slate-600 mt-1">Approved orders in the system</p>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent>
            ...
            <p className="text-xs text-slate-600 mt-1">Points per approved order</p>
          </CardContent>
        </Card>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-8">
        <Card>
          <CardHeader>
            <CardTitle>7-day Earnings Trend</CardTitle>
            <CardDescription>Approved order revenue trend</CardDescription>
          </CardHeader>
          <CardContent>
            {sevenDayTrend.length === 0 ? (
              <div className="py-12 text-center text-slate-600">No trend data available.</div>
            ) : (
              <ResponsiveContainer width="100%" height={300}>
                <LineChart data={sevenDayTrend}>
                  ...
                  <Line type="monotone" dataKey="approvedOrders" stroke="#3b82f6" strokeWidth={2} name="Approved Orders" />
                </LineChart>
              </ResponsiveContainer>
            )}
          </CardContent>
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>Approved orders and revenue over the last week</CardDescription>
          </CardHeader>
          <CardContent>
            {sevenDayTrend.length === 0 ? (
              <div className="py-12 text-center text-slate-600">No seller performance data yet.</div>
            ) : (
                ...
                {sevenDayTrend.map((point) => (
                  <div key={point.date} className="flex items-center justify-between rounded-lg border border-slate-200 bg-slate-50 p-4">
                    <div>
                      ...
                      <p className="text-xs text-slate-500">{point.approvedOrders} approved orders</p>
                    </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-[1.5fr_1fr] gap-8 mb-8">
        <Card>
          <CardHeader>
            ...
            <CardDescription>Top sellers by earnings</CardDescription>
          </CardHeader>
          <CardContent className="overflow-x-auto">
            {leaderboardError ? (
              <div className="py-12 text-center text-slate-600">{leaderboardError}</div>
            ) : leaderboard.length === 0 ? (
              <div className="py-12 text-center text-slate-600">No seller leaderboard data available.</div>
            ) : (
              <table className="min-w-full text-left text-sm">
                <thead>
                  <tr>
                    <th className="border-b px-4 py-3 font-medium text-slate-600">Rank</th>
                    <th className="border-b px-4 py-3 font-medium text-slate-600">Seller</th>
                    <th className="border-b px-4 py-3 font-medium text-slate-600">Earnings</th>
                    <th className="border-b px-4 py-3 font-medium text-slate-600">Level</th>
                    <th className="border-b px-4 py-3 font-medium text-slate-600">Badges</th>
                    <th className="border-b px-4 py-3 font-medium text-slate-600">Completed Orders</th>
                  </tr>
                </thead>
                <tbody>
                  {leaderboard.map((item) => (
                    <tr key={item.sellerId} className="border-b last:border-b-0 hover:bg-slate-50">
                      ...
                      <td className="px-4 py-3 text-slate-900">{item.level ?? 'Beginner'}</td>
        ...
        <Card>
          <CardHeader>
            <CardTitle>Top Sellers by Completed Orders</CardTitle>
            <CardDescription>Gamify seller performance</CardDescription>
          </CardHeader>
          <CardContent className="space-y-3">
            {topCompleted.length === 0 ? (
              <div className="py-12 text-center text-slate-600">No completed order leaderboard yet.</div>
            ) : (
              topCompleted.slice(0, 5).map((item) => (
                <div key={item.sellerId} className="flex items-center justify-between rounded-lg border border-slate-200 p-4">
                  <div>
                    ...
                    <p className="text-sm text-slate-600">{item.completedOrders} completed</p>
                  </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-8">
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>Weekly registration breakdown</CardDescription>
          </CardHeader>
          <CardContent>
            <ResponsiveContainer width="100%" height={300}>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <Card className="lg:col-span-1">
          <CardHeader>
            ...
            <CardDescription>Task distribution by game</CardDescription>
          </CardHeader>
          <CardContent>
            <ResponsiveContainer width="100%" height={300}>
              <PieChart>
